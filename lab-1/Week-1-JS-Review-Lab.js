 ...
 */
//WRITE YOUR CODE BELOW
//change price from string to number
const realtorFees = (listing) => {
const price = Number(listing.price.replace("$","").replace(",",""))
...
//WRITE YOUR CODE BELOW
//change price from string to number
const listingAscendingly = listings.slice().sort((listingA,ListingB) => {
const A = Number(listingA.price.replace("$","").replace(",","")) 
const B = Number(ListingB.price.replace("$","").replace(",",""))
return A - B
})
 ...
 */
//WRITE YOUR CODE BELOW
const newBuiltListings = listings.filter((building) => Number(building.propertySummary.builtIn) >= 1990 )
